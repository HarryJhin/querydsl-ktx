...
풀 base class로, 모든 Extension 인터페이스를 구현하고 페이지네이션/정렬/DML 헬퍼를 제공합니다.
...
class MyRepository : QuerydslSupport<MyEntity>(MyEntity::class.java) {
    // select, paging, slicing, modifying 등 사용 가능
    // infix 확장은 포함되지 않음
}
```
...
bulk DML 실행 시 영속성 컨텍스트를 관리합니다. `@Modifying`과 동일한 플래그를 지원합니다.

```kotlin
// 기본: flush before + clear after
